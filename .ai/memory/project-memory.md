...
exposes non-secret APM metadata to agents. Its primary differentiator is measurable token economy:
every compact map reports the estimated source context avoided and keeps analysis-cache performance
separate from model-token reduction.
...
- `node src/cli.js economy .`: display detailed token-economy metrics.
- `node src/cli.js generate skill <name> .`: create an editable, version-aware AI artifact.
